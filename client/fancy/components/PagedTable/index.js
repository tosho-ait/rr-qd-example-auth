    ...
    _buildHeader() {
        if (this.props.header) {
            ...
            if (this.props.header[0] && this.props.header[0].map) {
                ...
                headerRows.push(<tr>{headerItems}</tr>)
            }
    ...
    _buildBody() {
        ...
        body.map((row, index) => {
            if (index >= bt && index < tp) {
                ...
                let rowContent = row.cells.map((cell, cellIndex) => <td key={cellIndex} colSpan={cell.colSpan}
                                                                        class={cell.classes}
                    ...
                    if (row.childRows && row.childRows.length > 0) {
                        if (this.state.open[displayedRow]) {
                            row.childRows.map((childRow, crIndex) => {
                                let rowContent = childRow.cells.map((cell, cellIndex) => <td key={cellIndex}
